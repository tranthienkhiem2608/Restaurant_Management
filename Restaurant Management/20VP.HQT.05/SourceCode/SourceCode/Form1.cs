...
namespace SourceCode
{
    public partial class Form1 : Form
    {
        ...
        {
            connect = new SqlConnection(@"Data Source=DESKTOP-2V7JI18\MAYAO;Initial Catalog=QL_DH_GH_ONLINE;Integrated Security=True");
            connect.Open();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sqlselect = "SELECT * FROM dbo.LOGIN WHERE USERNAME = '" + username + "' AND PASSWORD = '" + password + "'";
            SqlDataAdapter sda = new SqlDataAdapter(sqlselect, connect);
            DataTable dt = new DataTable();
            sda.Fill(dt);

            string sqlselect1 = "SELECT USERID FROM LOGIN WHERE USERNAME = '" + username + "' AND PASSWORD = '" + password + "'";
            SqlCommand cmd1 = new SqlCommand(sqlselect1, connect);
            string id = Convert.ToString(cmd1.ExecuteScalar());

            string sqlselect5 = "SELECT ROLE_USER FROM LOGIN WHERE USERNAME = '" + username + "' AND PASSWORD = '" + password + "'";
            SqlCommand cmd5 = new SqlCommand(sqlselect5, connect);
            string role = Convert.ToString(cmd5.ExecuteScalar());
